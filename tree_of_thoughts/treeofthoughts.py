...
class MonteCarloSearch(TreeofThoughts):
    ...
    def monte_carlo_search_feature_generation(
        ...
        """
        # current_states = [initial_prompt]
        current_states = [(initial_prompt, trainingset, validationset, testset, feature_names)]
        state_values = {}
        ...
        for step in range(1, max_steps + 1):
            ...
            for state, current_trainingset, current_validationset,current_testset, current_feature_names in current_states:

                thoughts = self.model.generate_thoughts(
                    state, num_thoughts, initial_prompt, operations, operation_examples, current_feature_names, rejected_solutions
                )
                # Loop over each generated thought for further processing

                formated_thoughts = self.model.format_states(thoughts,current_feature_names)
                ...
                evaluated_thoughts = self.model.evaluate_states_feature_generation(
                    formated_thoughts, current_trainingset, current_validationset,current_testset, initial_metrics, operations,
                    current_feature_names, model_params
                )


                for thought, (new_trainingset, new_validationset, new_testset, new_feature_names, value, metrics_new, metrics_new_test) in evaluated_thoughts.items():
                    ...
                    if (
                        ...
                        if state == initial_prompt and ucb1_value >= pruning_threshold:
                            ...
                            selected_states.append(
                                (flattened_state, new_trainingset, new_validationset, new_testset, new_feature_names))
                            state_values[flattened_state] = value
                            print(f"Selected state: {flattened_state}, with improvements -")
                            print(f"    Accuracy New: {metrics_new[0]:.4f}")
                            print(f"    Precision New: {metrics_new[1]:.4f}")
                            print(f"    Recall New: {metrics_new[2]:.4f}")
                            print(f"    F1 New: {metrics_new[3]:.4f}")

                            print(f"    Accuracy Test: {metrics_new_test[0]:.4f}")
                            print(f"    Precision Test: {metrics_new_test[1]:.4f}")
                            print(f"    Recall Test: {metrics_new_test[2]:.4f}")
                            print(f"    F1 Test: {metrics_new_test[3]:.4f}")
                            if value > best_value:
                                best_state = flattened_state
                                best_matrix = metrics_new_test
                                best_value = value
        ...
        print(f"Selected best state: {best_state}, with improvements -")

        print(f"    Accuracy Test: {best_matrix[0]:.4f}")
        print(f"    Precision Test: {best_matrix[1]:.4f}")
        print(f"    Recall Test: {best_matrix[2]:.4f}")
        print(f"    F1 Test: {best_matrix[3]:.4f}")
